...
namespace Reflecta2._0
{
    ...
    class ADatabase
    {
        ...
        public ADatabase(string databaseName)
        {
            ...
            csb.Database = @"data\\"+databaseName+".fdb";
        ...
        private void CreateTables(out string sql, out FbCommand command)
        {
            ...
            sql = "CREATE TABLE Experiments (ExperimentID INTEGER PRIMARY KEY NOT NULL, SysChannelID INTEGER NOT NULL, ExperimentGroupsID INTEGER NOT NULL, SensorSerial VARCHAR(20) NOT NULL, SensorFirmware DECIMAL(3,3) NOT NULL, CalibrationValue FLOAT, FOREIGN KEY (SysChannelID) REFERENCES Channels(SysChannelID),  FOREIGN KEY (ExperimentGroupsID) REFERENCES ExperimentGroups(ExperimentGroupsID))";
            command = new FbCommand(sql, con);
            command.ExecuteNonQuery();            
        ...
        {
            string sql = "CREATE TABLE CalibrationGroup (CalibrationGroupID INTEGER PRIMARY KEY NOT NULL, NumOfAverage INTEGER NOT NULL, Datetime TIMESTAMP NOT NULL, SensorFirmware FLOAT NOT NULL, SensorSerial VARCHAR(20) NOT NULL )";
            FbCommand command = new FbCommand(sql, con);
            command.ExecuteNonQuery();

            sql = "CREATE TABLE Calibration (CalibrationID INTEGER PRIMARY KEY NOT NULL, RealValue FLOAT NOT NULL, MeasureValue Float NOT NULL, CalibrationGroup INTEGER NOT NULL,  FOREIGN KEY (CalibrationGroup) REFERENCES CalibrationGroup(CalibrationGroupID))";
            command = new FbCommand(sql, con);
            command.ExecuteNonQuery();

            sql = "CREATE GENERATOR generator_CalibrationID";
            command = new FbCommand(sql, con);
            command.ExecuteNonQuery();

            sql = "CREATE TRIGGER CalibrationID_trigger FOR Calibration ACTIVE BEFORE INSERT POSITION 0 AS BEGIN if (new.CalibrationID is null ) then new.CalibrationID = gen_id (generator_CalibrationID, 1); END";
            command = new FbCommand(sql, con);
            command.ExecuteNonQuery();

            sql = "CREATE GENERATOR generator_CalibrationGroupID";
            command = new FbCommand(sql, con);
            command.ExecuteNonQuery();

            sql = "CREATE TRIGGER CalibrationGroupID_trigger FOR CalibrationGroup ACTIVE BEFORE INSERT POSITION 0 AS BEGIN if (new.CalibrationGroupID is null ) then new.CalibrationGroupID = gen_id (generator_CalibrationGroupID, 1); END";
            command = new FbCommand(sql, con);
            command.ExecuteNonQuery();         
        }
    }
