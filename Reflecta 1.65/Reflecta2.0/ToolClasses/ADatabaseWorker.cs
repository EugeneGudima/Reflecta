...
namespace Reflecta2._0
{
    class ADatabaseWorker
    {
        ...
        {
            string sql = "INSERT INTO Reflectograms (ExperimentID, Datetime, ReflData) VALUES (" + expID + ",'" + dt.ToString("dd.MM.yy HH:mm:ss.ff") + "','" + ConvertToString(reflectogram) + "')";
            FbCommand command = new FbCommand(sql, db.Connection);
            command.ExecuteNonQuery();
        }
        ...
        internal static List<int> GetAllParametersIDs(ADatabase db)
        {
            ...
            while (myreader.Read())
            {
                if (myreader.FieldCount > 0)
                    result.Add(myreader.GetInt32(0));
            }
